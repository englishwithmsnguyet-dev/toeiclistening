...
def inject_tts_all(texts):
    # This function looks through the list of texts for a slide
    # and tries to inject TTS for both the main English word AND the phrase.
    res = []
    
    # Heuristic: the English word is usually wrapped in <span style="color: #0070C0;"><strong>...</strong></span>
    # The phrase is usually just <strong>...</strong> or just text at the end.
    
    for t in texts:
        t_str = str(t)
        
        # 1. Inject TTS into the main blue word (if present)
        def repl_word(m):
            full_span = m.group(0)
        ...
        t_injected = re.sub(r'<span style="color: #0070C0;"><strong>(.*?)</strong></span>', repl_word, t_str)
        
        # 2. Inject TTS into the phrase!
        # The phrase is usually the last item. It might be <strong>phrase</strong>
        # Let's check if it has NO color span but has <strong> or is just text.
        # Actually, let's just find the last text element in a block and assume it's the phrase?
        # A better way: if the text DOES NOT contain #0070C0, #00B050 (green phonetic), #7030A0 (purple viet), #FF0000 (red title), it might be the phrase.
        is_phrase = False
        if '#0070C0' not in t_str and '#00B050' not in t_str and '#7030A0' not in t_str and '#FF0000' not in t_str:
            clean_t = clean_text(t_str)
            # Phrases are usually English, > 1 word (e.g. "standing in line", "facing each other")
            # Exclude Vietnamese words or empty strings
            if clean_t and len(clean_t.split()) > 1 and not any(c in clean_t for c in 'áàảãạâấầẩẫậăắằẳẵặéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđ'):
                is_phrase = True
                
        if is_phrase:
            phrase_text = clean_text(t_str).replace('"', '&quot;').replace('`', '')
            tts_btn_phrase = f' <span onclick="playTTS(this.dataset.text, event)" data-text="{phrase_text}" style="cursor: pointer; opacity: 0.5; font-size: 0.9em; margin-left: 4px; display: inline-block;" onmouseover="this.style.opacity=\'1\'" onmouseout="this.style.opacity=\'0.5\'" title="Đọc cả cụm">🔊</span>'
            # If it's wrapped in <strong>, put it after the <strong>
            if '<strong>' in t_injected and '</strong>' in t_injected:
                t_injected = t_injected.replace('</strong>', f'</strong>{tts_btn_phrase}', 1)
            else:
                t_injected += tts_btn_phrase
                
        res.append(t_injected)
    return res

def format_text_block(texts):
    res = ""
    for t in texts:
        res += f'<div style="margin-bottom: 12px; font-size: 1.3rem;">{t}</div>'
    return res
...
while i < len(dang2_raw):
    ...
    # Fix Slide 9 missing image (it had 0 images, but it's about "face")
    if len(imgs) == 0 and len(texts) == 5 and 'CÁC ĐỘNG TỪ' in first_text:
        imgs = ['slide_118_img_4.jpg'] # Use the same image as Slide 8 "chat/talk"
        
    # Fix Slide 11 double image (it had 2 images for 'stand', same visual)
    if len(imgs) == 2 and len(texts) == 10 and 'stand' in clean_text(texts[2]):
        imgs = [imgs[0]]
        texts = texts[:4] + texts[6:7] + texts[8:9] # Keep Title, EN1, Pho1, VI1, Phrase1

    # Fix Slide 17 double image (it had 2 images for 'descend')
    if len(imgs) == 2 and len(texts) == 4 and 'descend' in clean_text(texts[1]):
        imgs = [imgs[0]]
        # texts is already Title, EN+Pho, VI, Phrase (4 items), perfect for 1-image layout!

    # Now handle standard 2-image slides
    if len(imgs) == 2:
        injected = inject_tts_all(texts)
        title = injected[0] if len(injected) > 0 else ""
        ...
        if len(texts) == 5:
            ...
            half = len(texts) // 2
            left_texts = injected[1:half+1]
            right_texts = injected[half+1:]
...
<div style="display: flex; flex-direction: row; gap: 32px; justify-content: center; width: 100%; align-items: flex-start;">
  ...
  <div style="flex: 1; text-align: center; display: flex; flex-direction: column; align-items: center;">
    ...
    <div style="color: var(--text-main); text-align: center;">{format_text_block(right_texts)}</div>
        ...
        slide['text'] = [html]
        slide['images'] = []
        # Clear slide audio if it exists in theory slides (only practice should have audio)
        if 'audio' in slide: del slide['audio']
        new_theory.append(slide)
        i += 1
        continue

    # Single-slide practice (Picture 2A, 2B)
    ...
    # Split-slide practice (Image slide + Text slide)
    if len(imgs) == 1 and len(texts) == 0:
        if i + 1 < len(dang2_raw) and 'PICTURE' in clean_text(dang2_raw[i+1].get('text',[''])[0]):
            ...
            # Carry over audio from the next slide (text slide) if it has one!
            # The text slide often has the audio!
            if next_slide.get('audio'):
                slide['practice']['audio'] = next_slide['audio']
            
            new_theory.append(slide)
            i += 2
            continue
            
    # Standard theory slides (1 image) - now this includes Slide 9, 11, 17 too!
    if 'PICTURE' not in first_text and 'PRACTICE' not in first_text:
        if len(imgs) == 1 and len(texts) > 0:
            injected = inject_tts_all(texts)
            title = injected[0] if len(injected) > 0 else ""
            body_texts = injected[1:] if len(injected) > 1 else []
            html = f"""<div style="text-align: center; margin-bottom: 24px; font-size: 1.5rem;">{title}</div>
<div style="display: flex; flex-direction: row; gap: 32px; align-items: center; justify-content: center; width: 100%;">
  ...
  <div style="flex: 1; color: var(--text-main);">
    {format_text_block(body_texts)}
            ...
            slide['text'] = [html]
            slide['images'] = []
            # Remove any stray audio from theory slides
            if 'audio' in slide: del slide['audio']
            new_theory.append(slide)
            i += 1
            continue
    ...
    # Fallback
    slide['text'] = inject_tts_all(texts)
    if 'audio' in slide: del slide['audio']
    new_theory.append(slide)
    i += 1
...
for slide in new_theory:
    if slide.get('practice'):
        ...
        # Make sure audio is set if it was missed
        # Wait, the split-slide logic handled audio transfer.
        pic_idx += 1
...
print("Successfully regenerated dang2 with all fixes!")
