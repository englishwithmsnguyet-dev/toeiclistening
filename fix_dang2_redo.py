...
# Read the ORIGINAL unformatted data to redo Dang 2
with open('data/part01_data.json', 'r', encoding='utf-8') as f:
    old_data = json.load(f)

# Read the CURRENT data (because it has Dang 1 formatted already)
...
# We will overwrite dang2 in current_data using old_data[2]
...
while i < len(dang2_raw):
    ...
    if len(slide.get('images', [])) == 2 and len(slide.get('text', [])) >= 5:
        ...
        # Check for swapped translations based on specific English keywords
  ...
  <div style="flex: 1; text-align: center; display: flex; flex-direction: column; align-items: center;">
    ...
    <div style="font-size: 1.3rem; line-height: 1.8; color: var(--text-main);">
      ...
      <div style="margin-bottom: 8px; color: var(--text-muted);">{vi2}</div>
        ...
        slide['text'] = [html]
        slide['images'] = [] # FIXED: CLEAR IMAGES ARRAY SO NO DUPLICATES!
        new_theory.append(slide)
        i += 1
        continue
...
print("FIXED DUPLICATES AND TRANSLATIONS!")
