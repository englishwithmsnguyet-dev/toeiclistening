...
test_pages = {}
for p in range(0, 146):
    pix = doc[p].get_pixmap(dpi=50) # lower DPI for faster OCR
    img_array = np.frombuffer(pix.samples, dtype=np.uint8).reshape(pix.height, pix.width, pix.n)
    gray = cv2.cvtColor(img_array, cv2.COLOR_BGR2GRAY) if pix.n == 3 else img_array
    text = pytesseract.image_to_string(gray)
    
    if "PART 1" in text:
        # Check if there is an example photo or Q1
        if "Directions" in text or "Example" in text or "1." in text:
            print(f"Page {p+1}: found PART 1!")
